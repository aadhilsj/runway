...
export const budgetsRepository={
  async getWorkspace(){const db=client();const userId=await requireAuthenticatedUserId(db);const results=await Promise.all([
    ...
    db.from("budget_actuals").select("*").eq("user_id",userId),
    ...
    for(const result of results)if(result.error)throw result.error;return{periods:results[0].data as BudgetPeriodRow[],lines:results[1].data as BudgetLineRow[],groups:results[2].data as Array<{id:string;name:string}>,groupCategories:results[3].data as Array<{group_id:string;category_id:string}>,actuals:results[4].data as Array<{category_id:string;month_start:string;actual_minor:number}>,categories:results[5].data as Array<{id:string;name:string;kind:string}>,currency:(results[6].data as {base_currency:string}).base_currency};
  },
  async createPeriod(monthStart:string,currency:string){const db=client();const userId=await requireAuthenticatedUserId(db);const{data,error}=await db.from("budget_periods").insert({user_id:userId,month_start:monthStart,currency,status:"planned"}).select("*").single();if(error)throw error;return data as BudgetPeriodRow;},
  async createLine(periodId:string,categoryId:string,budgetedMinor:number){const db=client();const userId=await requireAuthenticatedUserId(db);const{data,error}=await db.from("budget_lines").insert({user_id:userId,budget_period_id:periodId,category_id:categoryId,budgeted_minor:budgetedMinor}).select("*").single();if(error)throw error;return data as BudgetLineRow;},
  ...
  async deleteLine(id:string){const db=client();const userId=await requireAuthenticatedUserId(db);const{error}=await db.from("budget_lines").delete().eq("id",id).eq("user_id",userId);if(error)throw error;},
};
