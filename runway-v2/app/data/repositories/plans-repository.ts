...
export const plansRepository={
 ...
 async duplicatePlan(id:string){const db=client(),userId=await requireAuthenticatedUserId(db);const{data:source,error:sourceError}=await db.from("scenarios").select("*").eq("id",id).eq("user_id",userId).single();if(sourceError)throw sourceError;const{data:copy,error}=await db.from("scenarios").insert({user_id:userId,name:`${source.name} copy`,description:source.description,status:"draft"}).select("*").single();if(error)throw error;const{data:changes,error:changesError}=await db.from("scenario_changes").select("*").eq("scenario_id",id).eq("user_id",userId);if(changesError)throw changesError;if(changes?.length){const rows=changes.map(({id:_id,created_at:_created,updated_at:_updated,...row}:ScenarioChangeRow)=>({...row,scenario_id:copy.id}));const{error:insertError}=await db.from("scenario_changes").insert(rows);if(insertError)throw insertError;}return copy as PlanRow;},
