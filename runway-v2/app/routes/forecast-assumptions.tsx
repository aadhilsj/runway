...
export default function ForecastAssumptionsRoute() {
  ...
  return <Page eyebrow="What the forecast assumes" title="Forecast assumptions" description="Review the planned income and spending behind your chart. Changes here affect only the forecast—not the money in your accounts.">
    ...
    {data ? <>
      ...
      <section className="money-panel"><div className="panel-heading"><div><p className="section-kicker">Repeating automatically</p><h2>Recurring rules</h2></div><Link to="/settings/recurring">Manage rules →</Link></div><p className="muted">{data.rules.filter((rule) => rule.active && rule.kind === kind).length} active {kind} rule{data.rules.filter((rule) => rule.active && rule.kind === kind).length === 1 ? "" : "s"}. {kind === "income" ? "Reliable income may shorten the protection window, but is never counted as cash before receipt." : "Use a recurring rule when the same planned payment should repeat automatically."}</p></section>
      {planItems.length ? <section className="money-panel"><div className="panel-heading"><div><p className="section-kicker">Plans</p><h2>Scenario-only assumptions</h2></div><Link to="/plans">Open Plans →</Link></div><p className="muted">{planItems.length} item{planItems.length === 1 ? "" : "s"} are excluded from the base forecast until their Plan is selected.</p></section> : null}
