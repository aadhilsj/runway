...
export default function ForecastRoute() {
  ...
  return <Page eyebrow="What happens next" title="Forecast" description="Start with the cash you have now, then see how planned income and spending could change it. Planned money never changes your actual account balances.">
    ...
    {model ? <><section className="forecast-mental-model" aria-label="How this forecast is calculated"><div><span>Starting cash</span><strong>{money(model.summary.startingCashMinor, currency)}</strong><small>Actual money now</small></div><span aria-hidden="true">+</span><a href={`/forecast/assumptions?type=income&horizon=${months}`}><span>Planned income</span><strong>{money(model.summary.incomeMinor, currency)}</strong><small>Review what is included</small></a><span aria-hidden="true">−</span><a href={`/forecast/assumptions?type=expense&horizon=${months}`}><span>Planned spending</span><strong>{money(model.summary.expenseMinor, currency)}</strong><small>Review what is included</small></a><span aria-hidden="true">=</span><div><span>Cash in {months} months</span><strong>{money(model.summary.projectedBalanceMinor, currency)}</strong><small>{dateLabel(model.result.endDate)}</small></div></section>
      <section className="money-summary" aria-label="Forecast summary"><div className="metric-card"><p>Lowest cash balance</p><strong className={model.result.firstFloorBreach ? "negative" : ""}>{money(model.summary.lowestOperatingMinor, currency)}</strong><small>{dateLabel(model.result.lowestOperatingCash.date)}</small></div><div className="metric-card"><p>Planned income · next {months} months</p><strong className="positive">{money(model.summary.incomeMinor, currency)}</strong><a href={`/forecast/assumptions?type=income&horizon=${months}`}>Review income →</a></div><div className="metric-card"><p>Planned spending · next {months} months</p><strong>{money(model.summary.expenseMinor, currency)}</strong><a href={`/forecast/assumptions?type=expense&horizon=${months}`}>Review spending →</a></div><div className="metric-card"><p>Needs attention</p><strong>{model.summary.overdueCount}</strong><small>Overdue planned items</small></div></section>
      {workspace.data?.funds?.length ? <section className="actual-planned-strip"><div><strong>Fund projection</strong><span>{workspace.data.funds.length} funds tracked separately from account balances. Confirmed internal allocations leave cash and net worth unchanged.</span></div><div><strong>{model.result.projectedFundActions.length} projected actions</strong><span>Review the editable recommendation on the <a href="/funds/payday">Payday plan</a>.</span></div></section> : null}
      <section className="chart-card"><div className="panel-heading"><div><p className="section-kicker">Base forecast</p><h2>Your cash over the next {months} months</h2></div><span className="chart-legend">Cash available for everyday spending</span></div><div className="forecast-chart" aria-label="Projected operating and liquid cash chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={model.chart} margin={{top:12,right:20,bottom:8,left:8}}><CartesianGrid vertical={false} stroke="var(--color-line)"/><XAxis dataKey="date" tickFormatter={(value) => String(value).slice(5)} minTickGap={52}/><YAxis width={58} tickFormatter={(value) => formatMinorAxis(Number(value))}/><Tooltip formatter={(value) => money(Number(value), currency)} labelFormatter={(value) => dateLabel(String(value))}/><Area type="monotone" dataKey="operatingCashMinor" name="Cash" stroke="var(--color-accent)" strokeWidth={3} fill="rgb(187 95 67 / .10)"/></AreaChart></ResponsiveContainer></div></section>
      ...
      <section className="money-panel">
        <div className="panel-heading"><div><p className="section-kicker">What creates the chart</p><h2>Upcoming timeline</h2><span className="muted">Expected items only. Record real money in Activity when it happens.</span></div><div className="timeline-heading-actions"><button className="primary-button" type="button" onClick={() => { setEditingId(null); setDrawerOpen(true); }}>Add planned item</button></div></div>
        ...
        <div className="forecast-list">{model.timeline.map((item) => <article className="forecast-row forecast-row-detailed" key={item.id}>
