...
export default function FundsPaydayRoute() {
  ...
  return <Page eyebrow="Payday plan" title="Choose how to split available cash" description="Runway protects your current cash, expected bills, and operating floor first. You decide whether to use the suggested split.">
    ...
    {view ? <><section className="money-summary"><div className="metric-card"><p>Cash you have now</p><strong>{money(view.safeTrace.actualCashMinor, view.currency)}</strong></div><div className="metric-card"><p>Protected for bills</p><strong>{money(view.safeTrace.reservedObligationsMinor, view.currency)}</strong></div><div className="metric-card"><p>Protected minimum</p><strong>{money(view.safeTrace.operatingFloorMinor, view.currency)}</strong></div><div className="metric-card"><p>Available to split</p><strong>{money(view.safeTrace.safeToSpendMinor, view.currency)}</strong></div></section>
      <section className="money-panel"><div className="panel-heading"><div><p className="section-kicker">Suggested split</p><h2>Recommendation</h2><span className="muted">{recommendedTotal ? `Put ${money(recommendedTotal, view.currency)} toward your goals` : "Keep the cash available for now"}</span></div></div>
        <div className="payday-list">{view.result.recommendations.map((row) => <article className="payday-row" key={row.itemId}><div><strong>{row.label}</strong><p>{row.recommendedMinor > 0 ? `Suggested: ${money(row.recommendedMinor, view.currency)}` : "No amount is available after protecting bills and your minimum balance."}</p>{row.destinationType === "account" ? <small>A real bank transfer will still be required.</small> : null}</div><label>Amount<input aria-label={`${row.label} amount`} inputMode="decimal" value={amounts[row.itemId] ?? "0"} onChange={(event) => setAmounts((current) => ({ ...current, [row.itemId]: event.target.value }))}/><span>{view.currency}</span></label></article>)}</div>
