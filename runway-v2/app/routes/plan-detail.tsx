...
export default function PlanDetailRoute() {
  ...
  const [preview, setPreview] = useState<Awaited<ReturnType<typeof plansRepository.previewApply>> | null>(null);
  ...
  const previewApply = useMutation({ mutationFn: () => plansRepository.previewApply(planId!), onSuccess: setPreview, onError: value => setError(value instanceof Error ? value.message : "Preview failed") });
  const apply = useMutation({ mutationFn: () => plansRepository.applyToBase(planId!, preview!.confirmation_token), onSuccess: async () => { setPreview(null); setNotice("Plan items added to your main forecast."); await refresh(); }, onError: value => setError(value instanceof Error ? value.message : "Nothing changed") });
  ...
  return <Page eyebrow="Plan detail" title={plan.name} description={plan.description || "See the numbers behind this Plan."}>
    ...
    <section className="money-panel compact-apply-panel"><div className="panel-heading"><div><p className="section-kicker">Use this Plan</p><h2>Add it to your main forecast</h2></div><Link to="/plans/compare">Compare first →</Link></div>{notice ? <p className="form-notice">{notice}</p> : null}{!locked && !plan.legacy_source_id ? <button className="primary-button" onClick={() => previewApply.mutate()}>Preview changes</button> : null}{preview ? <div className="apply-preview"><h3>Add “{preview.name}”?</h3><p>{preview.change_count} item{preview.change_count === 1 ? "" : "s"}. Your actual balance will not change.</p><div className="row-actions"><button onClick={() => setPreview(null)}>Cancel</button><button className="primary-button" onClick={() => apply.mutate()} disabled={apply.isPending}>Confirm</button></div></div> : null}</section>
