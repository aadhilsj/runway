...
export default function PlansRoute() {
  ...
  const toggle = useMutation({ mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) => plansRepository.updatePlan(id, { comparison_enabled: enabled }), onSuccess: refresh });
  ...
  return <Page eyebrow="What if?" title="Plans" description="Try a future decision—such as a trip or a move—and compare its effect without changing your real money.">
    ...
    <section className="money-panel compact-plan-panel"><div className="panel-heading"><div><p className="section-kicker">Active plans</p><h2>What you are exploring</h2></div></div><div className="plan-list">{active.map(plan => {
      ...
      return <article className="plan-row" key={plan.id}><div className="plan-row-copy"><h3>{plan.name}</h3>{plan.description ? <p>{plan.description}</p> : null}<small>{count} item{count === 1 ? "" : "s"}</small></div><div className="plan-value"><span>Total change</span><strong className={effect < 0 ? "negative" : effect > 0 ? "positive" : ""}>{effect > 0 ? "+" : ""}{money(effect, currency)}</strong>{rules.length ? <small>plus {rules.length} repeating item{rules.length === 1 ? "" : "s"}</small> : null}</div><div className="plan-actions"><label><input type="checkbox" checked={plan.comparison_enabled} onChange={event => toggle.mutate({ id: plan.id, enabled: event.target.checked })}/> Compare</label><div className="row-actions"><Link to={`/plans/${plan.id}`}>View or edit</Link><button className="quiet-danger" type="button" onClick={() => setDeleting(plan)}>Delete</button></div></div></article>;
