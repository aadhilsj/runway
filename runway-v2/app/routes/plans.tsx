...
export default function PlansRoute() {
  ...
  const toggle = useMutation({ mutationFn: ({ id, enabled }: { id: string; enabled: boolean }) => plansRepository.updatePlan(id, { comparison_enabled: enabled }),
    ...
    onSuccess: async (_data, { id }) => { await refresh(); setComparisonOverrides((current) => { const next = { ...current }; delete next[id]; return next; }); },
    onError: (_error, { id }) => setComparisonOverrides((current) => { const next = { ...current }; delete next[id]; return next; }),
  });
  ...
  return <Page eyebrow="What if?" title="Plans" description="Try a future decision—such as a trip or a move—and compare its effect without changing your real money.">
    ...
    <section className="money-panel compact-plan-panel"><div className="panel-heading"><div><p className="section-kicker">Active plans</p><h2>What you are exploring</h2></div></div><div className="plan-list">{active.map(plan => {
      ...
      return <article className="plan-row" key={plan.id}><div className="plan-row-copy"><h3>{plan.name}</h3>{plan.description ? <p>{plan.description}</p> : null}<small>{count} item{count === 1 ? "" : "s"}</small></div><div className="plan-value"><span>Total change</span><strong className={effect < 0 ? "negative" : effect > 0 ? "positive" : ""}>{effect > 0 ? "+" : ""}{money(effect, currency)}</strong>{rules.length ? <small>plus {rules.length} repeating item{rules.length === 1 ? "" : "s"}</small> : null}</div><div className="plan-actions"><Link className="plan-open-link" to={`/plans/${plan.id}`}>Open</Link><label className="plan-compare"><input type="checkbox" checked={comparisonOverrides[plan.id] ?? plan.comparison_enabled} onChange={event => toggle.mutate({ id: plan.id, enabled: event.target.checked })}/> Compare</label><details className="plan-overflow"><summary aria-label={`More actions for ${plan.name}`}>•••</summary><div><button className="quiet-danger" type="button" onClick={() => setDeleting(plan)}>Delete plan</button></div></details></div></article>;
